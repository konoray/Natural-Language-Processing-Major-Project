...
print(f"对抗样本数据集大小: {len(adversarial_df)}")

# 准备数据
y_test = adversarial_df['is_fraud'].astype(int)
...
test_types = {
    ...
    '同义词替换': 'specific_dialogue_content_synonym',
    '句式转换': 'specific_dialogue_content_rephrase',
...
for test_name, text_column in test_types.items():
    ...
    # 准备文本数据
    X_test_text = adversarial_df[text_column].astype(str)
...
def plot_accuracy_comparison(results):
    ...
    plt.close()
    print("\n准确率对比图已保存为 model_accuracy_comparison.png")

# 生成详细报告
def generate_detailed_report(results, filename='adversarial_test_report.txt'):
